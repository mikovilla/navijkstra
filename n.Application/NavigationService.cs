        ...
        public static IEnumerable<string> GetShortestPath(Dictionary<string, Dictionary<string, int>> graph, string startNode, string endNode)
        {
            ...
            while (priorityQueue.Count > 0)
            {
                ...
                foreach (var (neighbor, weight) in graph[currentNode])
                {
                    ...
                    // If the calculated distance is shorter, update distance and previous node
                    if (tentativeDistance < distances[neighbor])
                    {
                        // Remove the neighbor if it's already in the priority queue (to reinsert with updated priority)
